...
class VAE_encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        mu = F.normalize(self.mu_layer(enc_h2), p=2, eps=1e-7)
        logvar = F.normalize(self.logvar_layer(enc_h2), p=2, eps=1e-7)
        return mu, logvar
...
class VAE_img(nn.Module):
    def __init__(self, n_enc_img_1, n_enc_img_2, n_dec_img_1, n_dec_img_2, n_input, n_z, n_clusters,
                 alpha):  # code_len,
    ...
    def pretrain_img(self, path='', train_loader=None, args=None):
        if args.pretrain:
            pretrain_vae_img(self.vae_img_enc, self.vae_img_dec, path, train_loader, args)
    ...
    def forward(self, x):
        ...
        return mu, logvar, x_bar, z, q

    def set_kappa(self, epoch):
        self.kappa = math.pow((1.0 * epoch + 1.0), 0.5)

class VAE_txt(nn.Module):
    def __init__(self, n_enc_txt_1, n_enc_txt_2, n_dec_txt_1, n_dec_txt_2, n_input, n_z, n_clusters, alpha):  # code_len,
    ...
    def pretrain_txt(self, path='', train_loader=None, args=None):
        if args.pretrain:
            pretrain_vae_txt(self.vae_txt_enc, self.vae_txt_dec, path, train_loader, args)
        # load pretrain weights
        checkpoint = torch.load(path, map_location='cuda:0')
...
def pretrain_vae_img(enc, dec, path, train_loader, args):
    print(enc)
    print(dec)
    optimizer = torch.optim.Adam(itertools.chain(enc.parameters(), dec.parameters()), lr=args.lr_pretrain_img)
    # index_list = [i for i in range(len(F_I))]
    for epoch in range(400):
        total_loss = 0.
        for batch_idx, (F_I, _, _, _) in enumerate(train_loader):
    ...
    print("img-->model saved to {}.".format(path))


def pretrain_vae_txt(enc, dec, path, train_loader, args):
    print(enc)
    print(dec)
    optimizer = torch.optim.Adam(itertools.chain(enc.parameters(), dec.parameters()), lr=args.lr_pretrain_txt)
    # index_list = [i for i in range(len(F_I))]
    for epoch in range(400):
        total_loss = 0.
        for batch_idx, (_, F_T, _, _) in enumerate(train_loader):
        ...
        state = {'enc': enc.state_dict(), 'dec': dec.state_dict()}
        torch.save(state, path)
    print("txt-->model saved to {}.".format(path))
